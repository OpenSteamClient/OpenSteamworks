...
public unsafe interface IClientCompat
{
    ...
    public void GetAvailableCompatToolsFiltered(CUtlStringList* compatTools, ERemoteStoragePlatform platform, int unk = 0);  // argc: 3, index: 4, ipc args: [bytes4, bytes4, bytes4], ipc returns: []
    [BlacklistedInCrossProcessIPC]
    ...
    // WARNING: Arguments are unknown!
    public void SpecifyCompatExperiment(AppId_t appid, string experiment, int unk);  // argc: 3, index: 7, ipc args: [bytes4, string, bytes4], ipc returns: []
    public bool BIsCompatibilityToolEnabled(AppId_t appid);  // argc: 1, index: 8, ipc args: [bytes4], ipc returns: [boolean]
    public string GetCompatToolName(AppId_t app);  // argc: 1, index: 9, ipc args: [bytes4], ipc returns: [string]
    public int GetCompatToolMappingPriority(AppId_t appid);  // argc: 1, index: 10, ipc args: [bytes4], ipc returns: [bytes4]
