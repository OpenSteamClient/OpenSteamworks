...
public:
    ...
    virtual unknown GetAllOwnedMultiplayerApps() = 0; //argc: 2, index 4
    virtual int GetAvailableLaunchOptions(AppId_t unAppID, int *outOptions, int optionsMax) = 0; //argc: 3, index 5
    virtual unknown GetAppDataSection(AppId_t appid, EAppInfoSection section, uint8 *buf, int bufMax, bool bSharedKVSymbols) = 0; //argc: 5, index 6
    virtual unknown GetMultipleAppDataSections(AppId_t appid, EAppInfoSection *pSections, int numSections, uint8 *buf, int bufMax, bool bSharedKVSymbols, int *outSectionLengths) = 0; //argc: 7, index 7
    ...
    virtual unknown BGetDLCDataByIndex(AppId_t appid, int iDLC, AppId_t *pDlcAppID, bool *pbAvailable, char *pchName, int cchNameBufferSize) = 0; //argc: 6, index 10
    virtual EAppType GetAppType(AppId_t appid) = 0; //argc: 1, index 11
    virtual bool TakeUpdateLock() = 0; //argc: 0, index 12
    virtual unknown GetAllAppsKVRaw() = 0; //argc: 3, index 13
