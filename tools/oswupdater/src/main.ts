...
export async function Main(protobufonly: boolean = false): Promise<number> {
    ...
    if (!protobufonly) {
        ...
        var interfaceConstsFilePath = `${projectDir}/OpenSteamworks.IPC/Interfaces/InterfaceConsts.cs`;
        ...
        ShimVersionInfo.CreateVersionFileFromManifest(versionHeader2, newManifest);
    } else {
    ...
    fs.rmSync(protocdircsharp, { recursive: true, force: true });
    mkdir(protocdircsharp);
